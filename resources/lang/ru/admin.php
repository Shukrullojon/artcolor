<?php

return [
    'sidebar' => [
        "our_team" => "Наша команда",
        "home" => "Главная",
        "slider" => "Слайдер",
        "language" => "Язык",
        "logout" => "Выйти",
        "add" => "добавлять",
        "edit" => "редактировать",
        "product" => "Продукция",
        "about_company" => "О компании",
        "news" => "Новости",
        "download" => "Скачать",
        "download_header" => "Заголовок",
        'count' => "Kоличество" ,
        'about_item' => 'About Item' ,
        'about_text' => 'О тексте' ,
        'info' => 'Информация' ,
        'additional' => "Дополнительная информация" ,
        'activity' => 'Мероприятия' ,
        "contact" => "Контакты",
        "header_text" => "Текст заголовка",
        "contact_type" => "Тип Контакта",
        "country" => "Страна",
        "footer_text" => "Текст нижнего колонтитула",
        "video" => "Bидео",
        'category' => "Категория",
        "sub_category" => "Подкатегория",
        "category_new" => "Новости категории",
        "product_type" => "Тип продукта",
        "product_type_item"=>"Элемент типа продукта",
        'contact' => "Контакт" ,
        "contact_header" => "Заголовок контакта" ,
        "contact_footer" => "Нижний колонтитул контакта" ,
        "category_text" => "Текст категории",
        "sub_category_header"=> "Заголовок подкатегории",
        "product_main" => "Основной продукт",
        "product_filter" => "Фильтр товаров",
        'card_header'  => "Заголовок карты" ,
        'card_about' => "Карточка О" ,
        'work' => "Наши работы" ,
        'work_item' => "Рабочий элемент",
        'service' => "Сервисы",
        'service_text' => "Сервисный текст" ,
        'service_header' => "Заголовок элемента услуги" ,
        'service_item' => "Пункт обслуживания" ,
        'service_image' => "Образ службы" ,
        "galery" => "Галерея",
        "galery_video" => "Галерея видео",
        "portfolio" => "Портфолио",
        "system" => "Система",
        "gallery_header" => "Заголовок галереи",
        "gallery_category" => "Категория галереи",
        "gallery_item_header" => "Заголовок элемента галереи",
        "gallery_filter" => "Фильтр галереи",
        "gallery_item" => "Элемент галереи",
        "gallery_item_footer" => "Текст элемента галереи",
        "gallery_about" => "Галерея о",
        "portfolio_about" => "Портфолио о",
        "portfolio_header" => "Название портфолио",
        "portfolio_image" => "Изображение портфолио",
        "portfolio_character" => "Персонаж портфолио",
        "portfolio_product" => "Продукт портфеля",
        "new_header" => "Название новости",
        "gallery_video_header" => "Заголовок",
        "gallery_video_filter" => "Фильтр",
        "gallery_video" => "Видео",
        "gallery_video_footer" => "Нижний заголовок",
        "download_about" => "Скачать о",
        "download_info" => "Информация",
        "download_category" => "Категория",
        'system_header' => "Системный заголовок" ,
        'system_text' => "Системный текст" ,
        'system_item_header' => "Заголовок системного элемента" ,
        'system_item' => "Системные элементы" ,
        'system_filter' => "Системный фильтр" ,
        'system_product' => "Системный продукт" ,
        'system_slider' => "Ползунок системы" ,
        'system_about' => "Система о" ,
        'about' => "О компании" ,
        'about_item' => "О товаре" ,
        'activity' => "Мероприятия" ,
        'videos' => "Видео" ,
        "download_item" => "Скачать элемент",
        "download_item_filter" => "Фильтр",
        "download_item_footer" => "Нижний заголовок",
        "download_item_header" => "Название заголовка",
        "download_item_product" => "Товар",
        'about_about' => "О нас",
        "branch" => "Ответвляться",
        "branch_header" => "Заголовок",
        "branch_about" => "Филиал о",
        "buypage" => "Страница покупки",
        "buypage_header" => "Заголовок",
    ],
    'crud' => [
        "effect" => "Эффект",
        "product" => "Товар",
        "type" => "Тип",
        "file" => "Файл",
        "social" => "Социальное",
        "link" =>"Url",
        "download" => "Скачать",
        "filter" => "Фильтр",
        "parent" => "Главный",
        "add" => "Добавлять",
        "save" => "Сохранять",
        "cancel" => "Отмена",
        "fio" => "Фио",
        "position" => "Позиция",
        "status" => "Статус",
        "image" => "Изображение",
        "active" => "Активный",
        "no_active" => "Нет активных",
        'title' => 'Заголовок',
        'short_title' => 'Краткое название' ,
        'text' => 'Полный текст' ,
        'right_text' => 'Правильный текст',
        "info" => "Информация",
        'video_image' => "Видео картинка" ,
        'video_link' => "Ссылка на видео" ,
        'update' => "Pедактировать" ,
        'about_item' => 'About Item' ,
        'count' => "Kоличество" ,
        'about_text' => 'About Text' ,
        'additional' => "Дополнительная информация" ,
        'activity' => 'Мероприятия' ,
        'short_text' => 'Короткий текст',
        "image_right" => "Изображение справа",
        "image_back" => "Изображение назад",
        "button" => "Кнопка",
        "button_url" => "URL кнопки",
        "button_target" => "Кнопка Target",
        "video" => "Bидео",
        'category' => "Категория" ,
        'category_name' => "Название категории" ,
        'news' => 'Новости' ,
        'slug' => "Slug" ,
        'contact' => "Контакт" ,
        'phone' => "Телефон" ,
        'email' => "Эл. адрес" ,
        'address' => "Aдрес" ,
        'logo' => "Логотип" ,
        'telegram' => "Телеграм" ,
        'instagram' => "Инстаграм" ,
        'youtube' => "YouTube" ,
        'facebook' => "Фейсбук" ,
        'timetable' => "Рабочее время" ,
        "contact_header" => "Заголовок контакта" ,
        "contact_footer" => "Нижний колонтитул контакта" ,
        "contact_type" => "Тип Контакта",
        "country" => "Страна",
        'timetable' => "Рабочее время",
        "items" => "Предметы",
        "colss" => "Крах",
        "application" => "Применение",
        "compound" => "Состав",
        "consumption" => "РАСХОД",
        "peculiarit" => "Особенности",
        "accordion_title" => "Название аккордеона",
        "accordion_info" => "Информация об аккордеоне",
        'card_header'  => "Заголовок карты" ,
        'card_about' => "Карточка О" ,
        "gallery_image" => "Галерея",
        "character" => "Характер",
        'work' => "Наши работы" ,
        'work_item' => "Рабочий элемент" ,
        'name' => "Hазвание" ,
        'region' => "Область" ,
        'product_image' => 'Изображение продукта' ,
        'image_small' => "Маленькое изображение" ,
        'service' => "Сервисы",
        'service_text' => "Сервисный текст" ,
        'service_header' => "Заголовок элемента услуги" ,
        'service_image' => "Образ службы" ,
        'image_small' => "Маленькое изображение",
        "price" => "Цена",
        "litr" => "Литр",
        "gallery" => "Галерея",
        'system_header' => "Системный заголовок" ,
        'system_text' => "Системный текст" ,
        'system_item_header' => "Заголовок системного элемента" ,
        'system_item' => "Системные элементы" ,
        'system_filter' => "Системный фильтр" ,
        'system_product' => "Системный продукт" ,
        'system_slider' => "Ползунок системы" ,
        'system_about' => "Система о" ,
        'system' => "Система" ,
        'about_about' => "О нас" ,
    ],
];
